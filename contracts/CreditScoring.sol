...
contract CreditScoring is AccessControl, ReentrancyGuard {
    ...
    event IndustryRiskMultiplierUpdated(
        IndustryCategory indexed industry,
        uint256 multiplier
    );
    ...
    function registerBusiness(
        ...
        require(
            businessProfiles[business].profileId == 0,
            "Business already registered"
        );
        require(
            businessIdToAddress[businessId] == address(0),
    ...
    function updatePaymentRecord(
        ...
        if (status == PaymentStatus.OnTime) {
            ...
            status == PaymentStatus.Late || status == PaymentStatus.VeryLate
        ) {
            ...
            if (profile.averagePaymentDelay == 0) {
                ...
                profile.averagePaymentDelay =
                    (profile.averagePaymentDelay *
                        (profile.totalPaymentsLate - 1) +
                        delayDays) /
                    profile.totalPaymentsLate;
            }
    ...
    function calculateCreditScore(
        ...
        if (profile.totalInvoices > 0) {
            paymentHistoryFactor =
                (profile.totalPaymentsOnTime * 100) /
        ...
        if (profile.totalInvoices > 0 && profile.totalDefaults > 0) {
            uint256 defaultRate = (profile.totalDefaults * 100) /
                profile.totalInvoices;
        ...
        industryFactor = industryMultiplier <= 100
            ? 100
            : (10000 / industryMultiplier);

        // Age factor - higher score for businesses with more invoices
        uint256 ageFactor = profile.totalInvoices >= 20
            ? 100
            : (profile.totalInvoices * 5);

        // Weighted score calculation
        uint256 weightedScore = ((paymentHistoryFactor * paymentHistoryWeight) +
            (defaultRateFactor * defaultRateWeight) +
            (delayFactor * delayFactorWeight) +
            (industryFactor * industryFactorWeight) +
            (ageFactor * ageFactorWeight)) / 100;
    ...
    function getRiskAssessment(
        ...
        external
        view
        returns (uint8 creditScore, uint8 riskLevel, uint256 recommendedRate)
    {
        require(
            businessProfiles[business].profileId > 0,
            "Business not registered"
    ...
    function calculateBaseRate(
        ...
        uint256[5] memory baseRates = [uint256(300), 500, 800, 1200, 2000]; // 3% to 20%
     ...
     */
    function getBusinessAtIndex(
        uint256 index
    ) external view returns (address businessAddress) {
        require(
            index < EnumerableSet.length(registeredBusinesses),
            "Index out of bounds"
        );
