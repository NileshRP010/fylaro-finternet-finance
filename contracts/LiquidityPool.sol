...
contract LiquidityPool is ReentrancyGuard, AccessControl {
    ...
    function deposit(
        ...
        require(
            config.maxDeposit == 0 || amount <= config.maxDeposit,
            "Amount above maximum deposit"
        );
        require(
            poolInfo.totalAssets + amount <= maxPoolSize,
    ...
    function withdraw(
        ...
        // Check lock period
        bool earlyWithdrawal = (block.timestamp - position.lastClaimTimestamp) <
            minLockPeriod;
    ...
    function financeInvoice(
        uint256 invoiceId,
        uint256 amount
    ) external onlyRole(MANAGER_ROLE) nonReentrant returns (bool success) {
        require(amount > 0, "Amount must be greater than 0");
        ...
        require(amount <= invoice.totalValue, "Amount exceeds invoice value");

        // Get invoice maturity date
        uint256 dueDate = invoice.dueDate;
        require(dueDate > block.timestamp, "Invoice already due");

        // Get issuer credit score
        (uint8 creditScore, , ) = creditScoring.getRiskAssessment(
            invoice.issuer
        );

        // Find appropriate strategy
        uint256 strategyId = _findSuitableStrategy(
            uint256(creditScore),
            dueDate,
            invoice.totalValue
        );
        ...
        // invoiceToken.purchaseInvoiceShares(invoiceId, amount); // TODO: Implement with ERC1155 transfers
    ...
    function updateStrategy(
        ...
        require(
            strategyId > 0 && strategyId <= strategyCount,
            "Invalid strategy ID"
        );
    ...
    function setMaxPoolSize(
        ...
        require(
            _maxPoolSize >= poolInfo.totalAssets,
    ...
    function getUserInfo(
        ...
        external
        view
        returns (uint256 lpTokens, uint256 pendingRewards, uint256 userShare)
    {
    ...
    function getActiveStrategies()
        external
        view
        returns (uint256[] memory activeStrategyIds)
    {
        uint256 count = 0;
    ...
    function _addStrategy(
        ...
        require(
            targetRiskLevel >= 1 && targetRiskLevel <= 10,
            "Invalid risk level"
        );
        require(
            minCreditScore >= 300 && minCreditScore <= 850,
    ...
    function _findSuitableStrategy(
        ...
        for (uint256 i = 1; i <= strategyCount; i++) {
            ...
            if (
                creditScore < strategy.minCreditScore ||
            ...
            uint256 creditScoreMatch = creditScore >= strategy.minCreditScore
                ? 100
                : 0;
    ...
    function _calculatePendingRewards(
        ...
        if (timeElapsed > 0) {
            uint256 userShare = (position.lpTokens * 10000) /
            ...
            uint256 newRewards = (userAssets * poolInfo.apy * timeElapsed) /
                (365 days * 10000);
    ...
    function _updateUtilizationRate() internal {
        ...
        poolInfo.utilizationRate =
            (poolInfo.totalValueFinanced * 10000) /
            poolInfo.totalAssets;
