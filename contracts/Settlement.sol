...
contract Settlement is ReentrancyGuard, Ownable {
    ...
    modifier validToken(uint256 _tokenId) {
        // Check if token exists by getting invoice info
        InvoiceToken.Invoice memory invoice = invoiceToken.getInvoice(_tokenId);
        require(invoice.issuer != address(0), "Token does not exist");
        _;
    }
    ...
    function depositPayment(
        ...
        require(msg.value >= invoice.totalValue, "Insufficient payment amount");
    ...
    function releasePayment(
        ...
        // Only debtor, token holder with balance > 0, or contract owner can release
        InvoiceToken.Invoice memory invoice = invoiceToken.getInvoice(_tokenId);
        require(
            msg.sender == invoice.debtor ||
                invoiceToken.balanceOf(msg.sender, _tokenId) > 0 ||
                msg.sender == owner(),
        ...
        invoiceToken.markInvoicePaid(_tokenId, paymentAmount);

        // Transfer payment to invoice issuer (since ERC1155 has multiple holders)
        address recipient = invoice.issuer;
    ...
    function autoReleasePayment(uint256 _tokenId) public validToken(_tokenId) {
        ...
        bool canAutoReleaseNow = (block.timestamp > invoice.dueDate + 7 days ||
            block.timestamp > deposit.depositTime + escrowTimeout);

        require(canAutoReleaseNow, "Conditions not met for auto-release");
        ...
        // Use already declared invoice variable to determine recipient
        address recipient = invoice.issuer;
        payable(recipient).transfer(paymentAmount);
        emit PaymentReleased(_tokenId, recipient, paymentAmount);
    }
