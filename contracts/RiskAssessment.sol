...
contract RiskAssessment is AccessControl, ReentrancyGuard {
    ...
    // Mappings
    mapping(uint256 => AssessmentResult) public assessments;
        ...
        returns (uint256 assessmentId)
    {
        ...
        address debtor = invoice.debtor;

        // Get invoice industry
        string memory industry = invoice.industry;

        // Get credit scores
        (uint8 issuerCreditScore, , ) = creditScoring.getRiskAssessment(
            invoice.issuer
        );
        (uint8 debtorCreditScore, , ) = creditScoring.getRiskAssessment(debtor);
        ...
        uint8 creditScoreComponent = _calculateCreditScoreComponent(
            uint256(issuerCreditScore),
            uint256(debtorCreditScore)
        );
        ...
        AssessmentResult memory assessment = AssessmentResult({
        ...
        // Update counters
        issuerAssessmentCount[invoice.issuer]++;
        debtorAssessmentCount[debtor]++;

        // Update counterparty risk
        _updateCounterpartyRisk(invoice.issuer, debtor, riskScore);
    ...
    function updateRiskAssessment(
        ...
        AssessmentResult storage assessment = assessments[assessmentId];
        ...
        returns (bool canFinance, uint256 maxAmount, uint256 interestRate)
    {
        ...
        AssessmentResult memory assessment = assessments[assessmentId];
